# The Fusor public API

The target, stated once: **a model crate ports from the legacy implementation
to Fusor by changing its imports.** Same shape — const generics plus type
parameters, infallible calls, and a flat crate root. The legacy API's warts
stay out, and the current compiler-backed design stays in.
...
```rust
use fusor::{Tensor, Device, Graph, Session, QMatrix, VarBuilder, Dim, Dtype, Error, Result};
```

| Item | Why it is public |
| --- | --- |
| `Tensor<const R: usize, T: Element = f32>` | The headline type. Rank and dtype in the type, every op infallible. Matches legacy Fusor's `Tensor<R, D, B>` in *shape* minus the `B: Fusion` slot, which had nothing to say once the e-graph owned materialization. |
| `Element` | The dtype witness (`f32`, `f16`, `bf16`, `u32`, `i32`). Needed to write a `T`-generic layer. |
| `Axis`, `Minus1`, `Minus2` | Axis selectors: `x.sum::<2>(Minus1)`. Strictly better than legacy Fusor's `D`/`Dim` marker zoo — `usize` also implements `Axis`, so `x.sum::<2>(1)` reads as it should. **Kept.** |
| `cat`, `stack` | Free functions over an `IntoIterator`, as legacy Fusor had them. `Tensor::cat` is the associated spelling. |
...
| `VarBuilder`, `ShardedVarBuilder` | GGUF weight loading. Re-exported from `fusor-gguf` so a model needs one dependency, not two. |
...
| --- | --- |
| the `typed-api` feature | **Deleted.** It swapped what `fusor::Tensor` and `fusor::Device` *named*, which made it non-additive: `--all-features` was not a valid configuration of the workspace (133 errors, measured). One root now, unconditionally. The `mod root` indirection, the two `#[cfg]` re-export arms and the two type-identity `const`s that policed them are gone with it. |
| `fusor::Tensor` = the runtime-rank tensor | It is `tensor::Dyn` now, and `fusor::Tensor` is the const-rank one. The struct was **renamed**, not aliased: a `pub(crate) type Tensor = Dyn` keeps the ~40 in-crate `use crate::tensor::Tensor` lines meaning what they meant, and puts nothing back on the public surface. |
...
| `Persistence`, `QFmt`, `QLayout` (root) | IR-level detail. No consumer imports them: the conformance suite takes them from `fusor_ir::dtype` directly, and models never spell them (a `QMatrix` gets its format from `Dtype::Q(fmt)` destructuring and from `RawTensorBytes`, both inferred). |
...
`use` of `fusor_ir::ir::launch::MaskKind`, so `attention_masked` took an
...
## 4. Not being added, from legacy Fusor

The point of the port is legacy Fusor's *ergonomics*, not its inventory.
...
- **`as_cpu_mut`, `unwrap_gpu`, backend juggling.** fusor has no CPU/GPU
...
deep for something legacy Fusor had as a method:

```rust
// today
let scores = fusor::composite::attention_masked(&q, &k, &v, scale, mask)?;
let (q, k) = fusor::composite::rope_pair(&q, &k, &cos, &sin, offset)?;
let up = fusor::composite::upsample_nearest2d(&x, 4)?;
...
Derived by intersecting every method call in the four model crates' compiled
source with the op surface of `Dyn`, then subtracting what `Tensor<R, T>`
already exposed.
...
`fusor-cpu` and `fusor-gpu` return `Error::Plan("OneHotContract is present
only as the candidate the cost model rejects; it lowers through Launch::Contract, not
here")`. So `fusor-tile`'s `SCATTER_ONE_HOT_CONTRACT` rule mints a candidate
