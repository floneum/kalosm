...
> `fusor-gpu/src/target.rs:197` `static LAST_EXIT is never used` and
> `fusor-gpu/src/target.rs:408` `unused variable: gap`.
...
|---|---|---|---|---|
| 1 | `graphvis_dot` + its subject test (`fusor-gpu/src/launch.rs:918`) | NCS (self-test only) | ~60 | low; **debug tooling** — confirm the DOT dumps of commit `4e3261a` don't route here (grep says they don't) |
| 2 | `check_output` + subject test (`fusor-conformance/src/goldens.rs:107`) | NCS (self-test only) | ~35 | low; output-hash goldens are otherwise unwired — decide wire-or-delete |
| 3 | `cases_from_rows` + subject test (`fusor-conformance/src/harness.rs:202`) | NCS (self-test only) | ~30 | very low |
| 4 | `d_expr` + subject test (`fusor-autograd/src/map_adjoint.rs:472`) | NCS (self-test only) | ~25 | low; reads like a custom-adjoint API entry — check intent |
| 5 | `run_case` (`fusor-conformance/src/harness.rs:482`) | NCS (only other mention is a string in `suite.rs:542`) | ~22 | very low; fix the error-message string |
| 6 | `fill_indices` + subject test (`fusor-conformance/src/harness.rs:341`) | NCS (self-test only) | ~20 | very low |
| 7 | `layout_elements` (`fusor-gpu/src/lower.rs:77`, already `#[allow(dead_code)]`) | NCS | ~15 | very low |
| 8 | `reverse_order` + subject test (`fusor-autograd/src/backward.rs:207`) | NCS (self-test only) | ~15 | very low |
| 9 | `TensorCache::fixed_windowed` (`fusor/src/cache/kv.rs:118`) | NCS (self-test only) | ~10 | low; the ring test rebuilds via the `fixed_named` + `window` path `KvCache::windowed` already uses. The ring feature itself is **live** (`models/kalosm-llama/src/raw/cache.rs:45`) |
| 10 | `Reverse::topology` accessor (`fusor-autograd/src/backward.rs:145`) | NCS (only other mention is inside an error string) | ~5 | very low |
| 11 | 10 unused dependency entries (list below) | DEP | 10 TOML | none — proven by removal + build |
| 12 | `fusor`'s `cpu`/`gpu` features (`fusor/Cargo.toml:26-29`) | FEAT | 4 TOML | none — zero `cfg(feature = "cpu"/"gpu")` anywhere in either workspace; removal + check green in both (pasted below) |
| 13 | `distribute_workgroups` twins (`fusor-cost/src/realize.rs:699` vs `fusor-gpu/src/lower.rs:257`) | DUP → A/B | ~20 net | medium: bodies differ in width (u64/u32) and z-clamping; consolidation must keep the **gpu** semantics bit-exact and be proven by `FUSOR_DUMP_PLAN` byte-identity, not by sampling |
...
```
fusor-conformance/Cargo.toml:16  fusor-autograd
fusor-conformance/Cargo.toml:19  fusor-gpu
fusor-conformance/Cargo.toml:20  fusor-cpu
fusor-conformance/Cargo.toml:21  smallvec
fusor-conformance/Cargo.toml:25  rand
fusor-cpu/Cargo.toml:14          fusor-cost
fusor-gguf/Cargo.toml:16         bytemuck
fusor-gpu/Cargo.toml:19          bytemuck
fusor-tile/Cargo.toml:15         fixedbitset
fusor-tile/Cargo.toml:16         half
```
...
$ cargo check --workspace --all-targets
    Checking fusor v0.1.0 (.../fusor)
    Checking fusor-conformance v0.1.0 (.../fusor-conformance)
    Finished `dev` profile [unoptimized + debuginfo] target(s) in 8.20s
```

Reverted after the experiment. Note `fusor-conformance` still reaches both
backends through `fusor` itself; dropping its direct deps changes no feature
resolution (the workspace has no non-default features in play).

### Item 12 — the `cpu`/`gpu` features

Declared and defaulted in `fusor/Cargo.toml`, they gate nothing: zero
...
```
$ cargo check --workspace --all-targets        (fusor repo)
...
`fusor-gpu/src/caps.rs`, and the vendored naga fork it pairs with is
...
return and unclamped `z`. `fusor-gpu` already depends on `fusor-cost`, so
the single home is `fusor-cost` carrying the gpu semantics; the cost model
then prices exactly the grid the backend launches, which is a small coherence
win on top of the deletion. Gate: `FUSOR_DUMP_PLAN` byte-identity over a full
...
* **`fusor-cpu` vs `fusor-gpu`/`fusor-tile` structure (angle 6)**: the
  fn-name intersection surfaced no body-level duplicates beyond items 13 and
  the rejected pairs above; `fusor-cpu/src/lower/contract.rs` (1,033 lines)
  parallels the gpu's (2,283) in shape but lowers to a different execution
  model (tape/registers, not naga). Serving both from `fusor-tile` is a
...
identical to HEAD, `FUSOR_VERIFY_MEMBERS=1` quantized 108/108, every crate's
...
difference. Nor are the kernels: `FUSOR_WGSL_DUMP` produced **506 shaders
