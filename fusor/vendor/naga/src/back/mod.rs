...
impl crate::Expression {
    ...
    pub const fn bake_ref_count(&self) -> usize {
        match *self {
            ...
            // FUSOR PATCH: a simdgroup load is a synchronization-sensitive
            // side effect like `Load`. Left unbaked at ref count 1 it is
            // emitted inline at its use site, which lets it sink past the
            // workgroup barrier its `Emit` statement was ordered against —
            // threads then read tiles their neighbours have not finished
            // staging, which is the coop matmul NaN flake.
