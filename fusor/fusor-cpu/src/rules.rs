...
use fusor_ir::ir::launch::{AccessPlan, Launch, MapDomain, MapTiling, Operand, ScheduleDomain};
use fusor_ir::ir::{Level, Node, Op, OpTag};
use fusor_ir::rule;
use fusor_ir::scalar::ScalarExpr;
use fusor_ir::shape::{Layout, MultiFlattenMap};
...
pub(crate) fn select_vector_width(
    b: &mut Builder<'_>,
    id: Id,
    node: &Node,
    f: &Facts<'_>,
...
pub(crate) fn access_contiguous(
    b: &mut Builder<'_>,
    id: Id,
    node: &Node,
    _f: &Facts<'_>,
...
pub(crate) fn access_unit_inner(
    b: &mut Builder<'_>,
    id: Id,
    node: &Node,
    _f: &Facts<'_>,
    ...
    mint_access(b, id, node, |o| {
        ...
        let unit_inner = strides
            .last()
            .is_some_and(|s| s.known_eq(fusor_ir::shape::Dim::Const(1)));
...
pub(crate) fn access_gather(
    ...
    let gathered = mint_access(b, id, node, |o| {
        (o.access != AccessPlan::Gather && !o.layout.is_contiguous()).then_some(AccessPlan::Gather)
    });
