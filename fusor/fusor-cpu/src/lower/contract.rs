...
use fusor_ir::ir::kernel::{
    Accumulator, Addr, Builtin, ElementType, KernelIr, LocalDecl, ScalarElement, Stmt, StorageView,
    TileExpr, TileExprKind, WorkgroupAxis,
};
...
use super::{Binds, Translate, bin, cmp, lit_f32, lit_u32, load, u32_ty};
...
fn collapsed_strides(layout: &Layout, groups: [u32; 3]) -> Option<[u32; 3]> {
    ...
    let ext_all: Vec<u32> = shape
        .iter()
        .map(|d| d.as_const().map(|v| v as u32))
        .collect::<Option<_>>()?;
    let str_all: Vec<u32> = strides
        .iter()
        .map(|d| d.as_const().map(|v| v as u32))
        .collect::<Option<_>>()?;
...
fn strided_index(x: (&TileExpr, u32), y: (&TileExpr, u32), z: (&TileExpr, u32)) -> TileExpr {
...
fn build(
    ...
    let tile = tile.fit(m, n, caps.limits.max_compute_invocations_per_workgroup);
    ...
    let row0 = bin(
        ...
        bin(
            BinOp::Mul,
            bin(BinOp::Div, lane.clone(), lit_u32(tile.col_groups), u32_ty()),
            lit_u32(tm),
            u32_ty(),
        ),
        u32_ty(),
    ...
    // in `fusor-tile` mints a non-contiguous `Alias` for any contraction
    ...
    let side_coords =
        |side: &ContractSide, groups: [u32; 3], flats: [&TileExpr; 3]| -> Option<Vec<TileExpr>> {
