# fusor — archived scaffold contracts
...
fusor-cost        -> ir
fusor-gguf        -> ir
fusor-gpu         -> ir, tile, cost, gguf
fusor-cpu         -> ir, tile, cost, gguf
fusor             -> all of the above
fusor-conformance -> all of the above
```
...
| `QFmt` (6), `QLayout` | `dtype` | `BlockSpec`, `BlockProgram`, `GgmlType` live in `fusor-gguf` |
...
// ir::kernel::ArenaPlanner — object-safe; one impl in fusor-tile
...
profitability judgement; put it in `fusor-cost` instead. Returning `None`
...
1. **`domains::{CoopDomain::legal, SgemmDomain::legal, ...}` became free
   ...
   `MapDomain` are defined in `fusor-ir`, so `fusor-tile` cannot add inherent
   methods to them (orphan rule). They are now
   `fusor_tile::domains::coop::legal(..)` etc., re-exported from `domains` as
...
4. **On-disk formats live in `fusor-cost` as mirror structs, never as serde
   derives on `fusor-ir` types.** `fusor-ir` has no `serde` dependency at all,
   ...
   set, letting a stale one outlive a driver update. `fusor-cost::tune_cache`
...
5. **The `rule!` macro ships only the declarative arm** described in §2.4. The
   ...
   already used by all eighteen `fusor-ir` rules, fifteen `fusor-tile` rules,
   three `fusor-autograd` rules and seven `fusor-cpu` rules, so it must keep
   working.
...
8. **`fusor-conformance` gained `src/main.rs`.** The crate is described as
...
10. **Each `fusor-gpu::lower::<family>` / `fusor-cpu::lower::<family>`
...
does not move to `fusor-ir`. Delete it, mint the alternative unconditionally,
and let the realized-DAG cost in `fusor-cost` reject it. Those four gates are
