use fusor::{
    layers::{BatchNorm1d, Conv1d, Conv1dConfig, Conv2d, Conv2dConfig, Embedding, LayerNorm, Linear},
...
fn rel_shift(x: &Tensor<4, f32>) -> Tensor<4, f32> {
    ...
    let reshaped = padded.reshape([batch, heads, pos_len + 1, q_len]).to_concrete();
    reshaped
        .narrow(2, 1, pos_len)
...
fn encoder_attention_mask(device: &Device, batch: usize, seq_len: usize, valid_len: usize) -> Tensor<4, f32> {
...
impl ConformerFeedForward {
    ...
    fn forward(&self, x: &Tensor<3, f32>) -> Tensor<3, f32> {
        self.linear2.forward(&self.linear1.forward(x).silu().to_concrete())
    }
}
...
impl ConformerConvolution {
    fn load(device: &Device, vb: &mut VarBuilder, d_model: usize, kernel_size: usize) -> Result<Self> {
        Ok(Self {
    ...
    fn forward(&self, x: &Tensor<3, f32>, valid_mask: Option<&Tensor<2, f32>>) -> Tensor<3, f32> {
        ...
        self.pointwise_conv2.forward(&x).transpose(1, 2).to_concrete()
    }
}
...
impl RelPositionMultiHeadAttention {
    fn load(device: &Device, vb: &mut VarBuilder, d_model: usize, num_heads: usize) -> Result<Self> {
    ...
    fn forward(&self, x: &Tensor<3, f32>, pos_emb: &Tensor<3, f32>, mask: Option<&Tensor<4, f32>>) -> Tensor<3, f32> {
        let [batch, time, _] = x.shape();
...
impl ConformerLayer {
    fn load(device: &Device, vb: &mut VarBuilder, cfg: &CohereEncoderConfig) -> Result<Self> {
        Ok(Self {
            ...
            self_attn: RelPositionMultiHeadAttention::load(device, &mut vb.pp("self_attn"), cfg.d_model, cfg.n_heads)?,
            norm_conv: LayerNorm::load(device, &mut vb.pp("norm_conv"), 1e-5)?,
            conv: ConformerConvolution::load(device, &mut vb.pp("conv"), cfg.d_model, cfg.conv_kernel_size)?,
    ...
    fn forward(&self, x: &Tensor<3, f32>, pos_emb: &Tensor<3, f32>, mask: Option<&Tensor<4, f32>>, valid_mask: Option<&Tensor<2, f32>>) -> Tensor<3, f32> {
        let residual = x.clone();
        let x = (residual + self.feed_forward1.forward(&self.norm_feed_forward1.forward(x)).mul_scalar(0.5))
            .to_concrete();
        let residual = x.clone();
        let x = (residual + self.self_attn.forward(&self.norm_self_att.forward(&x), pos_emb, mask)).to_concrete();
        let residual = x.clone();
        let x = (residual + self.conv.forward(&self.norm_conv.forward(&x), valid_mask)).to_concrete();
        let residual = x.clone();
        self.norm_out
            .forward(&(residual + self.feed_forward2.forward(&self.norm_feed_forward2.forward(&x)).mul_scalar(0.5)).to_concrete())
    }
}
...
impl ConformerEncoder {
    fn load(device: &Device, vb: &mut VarBuilder, cfg: &CohereEncoderConfig) -> Result<Self> {
        ...
        for i in 0..cfg.n_layers {
            layers.push(ConformerLayer::load(device, &mut vb.pp(format!("layers.{i}")), cfg)?);
        }
        Ok(Self {
...
impl DecoderAttention {
    fn load(device: &Device, vb: &mut VarBuilder, hidden_size: usize, num_heads: usize) -> Result<Self> {
    ...
    fn forward(&self, hidden_states: &Tensor<3, f32>, context_states: Option<&Tensor<3, f32>>, attention_mask: Option<&Tensor<4, f32>>) -> Tensor<3, f32> {
...
impl TransformerDecoderLayer {
    fn load(device: &Device, vb: &mut VarBuilder, cfg: &CohereDecoderConfig) -> Result<Self> {
        Ok(Self {
            ...
            first_sub_layer: DecoderAttention::load(device, &mut vb.pp("first_sub_layer"), cfg.hidden_size, cfg.num_attention_heads)?,
            layer_norm_2: LayerNorm::load(device, &mut vb.pp("layer_norm_2"), 1e-5)?,
            second_sub_layer: DecoderAttention::load(device, &mut vb.pp("second_sub_layer"), cfg.hidden_size, cfg.num_attention_heads)?,
            layer_norm_3: LayerNorm::load(device, &mut vb.pp("layer_norm_3"), 1e-5)?,
            third_sub_layer: DecoderFeedForward::load(device, &mut vb.pp("third_sub_layer"), &cfg.hidden_act)?,
        })
    }

    fn forward(&self, hidden_states: &Tensor<3, f32>, encoder_hidden_states: &Tensor<3, f32>, self_attention_mask: &Tensor<4, f32>, cross_attention_mask: Option<&Tensor<4, f32>>) -> Tensor<3, f32> {
        let residual = hidden_states.clone();
        let hidden_states = (residual + self.first_sub_layer.forward(&self.layer_norm_1.forward(hidden_states), None, Some(self_attention_mask))).to_concrete();
        let residual = hidden_states.clone();
        let hidden_states = (residual
            + self.second_sub_layer.forward(&self.layer_norm_2.forward(&hidden_states), Some(encoder_hidden_states), cross_attention_mask))
        .to_concrete();
        let residual = hidden_states.clone();
...
impl TransformerDecoderEmbedding {
    fn load(device: &Device, vb: &mut VarBuilder) -> Result<Self> {
        Ok(Self {
            ...
            position_embedding: FixedPositionalEncoding::load(device, &mut vb.pp("position_embedding"))?,
    ...
    fn forward(&self, input_ids: &Tensor<2, u32>, positions: &Tensor<2, u32>) -> Tensor<3, f32> {
        self.layer_norm
            .forward(&(self.token_embedding.forward(input_ids) + self.position_embedding.forward(positions)).to_concrete())
    }
}
...
impl TransformerDecoder {
    fn load(device: &Device, vb: &mut VarBuilder, cfg: &CohereDecoderConfig) -> Result<Self> {
        ...
        for i in 0..cfg.num_layers {
            layers.push(TransformerDecoderLayer::load(device, &mut vb.pp(format!("_decoder.layers.{i}")), cfg)?);
        }
        Ok(Self {
            embedding: TransformerDecoderEmbedding::load(device, &mut vb.pp("_embedding"))?,
            layers,
            final_layer_norm: LayerNorm::load(device, &mut vb.pp("_decoder.final_layer_norm"), 1e-5)?,
        })
    }

    fn forward(&self, input_ids: &Tensor<2, u32>, encoder_hidden_states: &Tensor<3, f32>, encoder_length: usize) -> Tensor<3, f32> {
...
mod tests {
    ...
    async fn debug_first_step_topk() {
        ...
        let wav = hound::WavReader::open(concat!(env!("CARGO_MANIFEST_DIR"), "/examples/samples_jfk.wav"))
            .unwrap()
            .into_samples::<i16>()
        ...
        let input_features = Tensor::from_slice(&device, [1, config.preprocessor.features, total_frames], &features);
        let prompt_ids = [7_u32, 4, 16, 62, 62, 5, 9, 11, 13];

        let mut conv = input_features.transpose(1, 2).unsqueeze(1).to_concrete();
        conv = model.encoder.pre_encode.conv0.forward(&conv).relu().to_concrete();
        conv = model.encoder.pre_encode.conv1_dw.forward(&conv).to_concrete();
        conv = model.encoder.pre_encode.conv1_pw.forward(&conv).relu().to_concrete();
        conv = model.encoder.pre_encode.conv2_dw.forward(&conv).to_concrete();
        conv = model.encoder.pre_encode.conv2_pw.forward(&conv).relu().to_concrete();
        let conv_slice = conv.clone().as_slice().await.unwrap();
        let conv_shape = conv_slice.shape();
        let (conv_b, conv_c, conv_t, conv_f) = (conv_shape[0], conv_shape[1], conv_shape[2], conv_shape[3]);
        ...
        let (pre_encode, pre_len) = model.encoder.pre_encode.forward(&input_features, valid_frames);
        ...
        let hidden_states = model.decoder.forward(&input_ids, &encoder_hidden_states, encoder_length);
        let last_hidden = hidden_states.narrow(1, prompt_ids.len() - 1, 1).to_concrete();
...
impl Cohere {
    pub fn load(device: &Device, vb: &mut VarBuilder, config: CohereConfig) -> Result<Self> {
        ...
        let decoder = TransformerDecoder::load(device, &mut vb.pp("transf_decoder"), &config.transf_decoder.config_dict)?;
        let encoder_decoder_proj = if config.encoder.d_model != config.transf_decoder.config_dict.hidden_size {
            Some(Linear::load(device, &mut vb.pp("encoder_decoder_proj"))?)
        } else {
            None
        };
        let lm_bias = vb.pp("log_softmax")
            .pp("mlp")
            .pp("layer0")
            .get("bias", device)?
            .dequantize();
    ...
    fn lm_head(&self, hidden_states: &Tensor<3, f32>) -> Tensor<3, f32> {
        hidden_states
            .q_mat_mul(self.decoder.embedding.token_embedding.embeddings_quantized())
            .add_(&self.lm_bias)
    }
    ...
    pub async fn generate_greedy(
        ...
        for _ in 0..max_new_tokens {
            ...
            let hidden_states = self.decoder.forward(&input_ids, &encoder_hidden_states, encoder_length);
