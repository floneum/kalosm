...
async fn run_test_suite(mut progress: Signal<TestProgressState>, mut result: Signal<RunResult>) {
    progress.set(TestProgressState::default());
    ...
    let suite_result =
        fusor_conformance::suite::webgpu::run_webgpu_kernel_suite_with_progress(&device, |case| {
            let mut progress = progress.write();
            mark_running_step(&mut progress, case);
        })
        .await;
