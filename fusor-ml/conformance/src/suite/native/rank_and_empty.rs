...
/// All rank-4 reductions/normalizations validated against an independently
/// computed host reference. Each sub-case keeps its own shape and tolerance:
///   * `sum` over each of the four axes,
///   * `mean` over axis 0,
///   * `max` / `min` over the last axis,
///   * `softmax_last_dim`,
///   * `rms_norm_fused`.
pub fn rank4_reductions_match_reference() -> AssertionCases {
    let mut assertions = AssertionCases::new();

    // sum over each axis (out rank 3).
    {
        ...
        for axis in 0..4 {
            ...
            assertions.push(
                ...
                .runs(1)
                .into_case(format!(
                    "rank_and_empty::rank4_reductions_match_reference::sum_axis{axis}"
                )),
            );
        }
    }

    // mean over axis 0 (out rank 3).
    {
        ...
        assertions.push(
            ...
            .compare_with(approx_compare::<3, f32>(1e-4))
            .runs(1)
            .into_case("rank_and_empty::rank4_reductions_match_reference::mean_axis0"),
        );
    }

    // max / min over the last axis (out rank 3).
    {
        ...
        assertions.push(
            ...
            .runs(1)
            .into_case("rank_and_empty::rank4_reductions_match_reference::max_last"),
        );

        assertions.push(
            fusor_conformance::assert(async |input: Tensor<4, f32>| {
                input.min::<3>(3).to_concrete()
            })
            .arg(move |device: &Device| Tensor::from_slice(device, SHAPE, &data))
            .equal_to(move |input: Tensor<4, f32>| {
            ...
            .compare_with(approx_compare::<3, f32>(1e-6))
            .runs(1)
            .into_case("rank_and_empty::rank4_reductions_match_reference::min_last"),
        );
    }

    // softmax over the last axis (out rank 4).
    {
        ...
        assertions.push(
            ...
            .runs(1)
            .into_case("rank_and_empty::rank4_reductions_match_reference::softmax_last_dim"),
        );
    }

    // fused RMS norm over the last axis (out rank 4).
    {
        ...
        assertions.push(
            fusor_conformance::assert(move |input: Tensor<4, f32>| {
                ...
                async move {
                    let w: Tensor<1, f32> =
                        Tensor::from_slice(&input.device(), [SHAPE[3]], &weight);
            ...
            .runs(1)
            .into_case("rank_and_empty::rank4_reductions_match_reference::rms_norm_fused"),
        );
    }
