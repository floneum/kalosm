...
impl PairedEpilogue {
    /// Build a paired epilogue from an arbitrary tile-IR closure that takes
    /// `(gate, up)` only (no extras). `label` appears in graph visualizations
    /// and kernel names; the structural hash of the closure's output drives
    /// pipeline caching, so two closures that produce identical Expr trees
    /// will share a pipeline regardless of label.
    ...
    {
        Self::with_full(label, None, 2 + extras_arity, build)
    }
    ...
    where
        F: Fn(Tile<1>, Tile<1>) -> Tile<1> + Send + Sync + 'static,
    {
        Self::with_full(label, cpu_preset, 2, move |tiles| {
            build(tiles[0].clone(), tiles[1].clone())
        })
    }
    ...
    where
        F: Fn(&[Tile<1>]) -> Tile<1> + Send + Sync + 'static,
    {
        assert!(arity >= 2, "paired epilogue must take at least gate and up");
    ...
    pub fn swiglu() -> Self {
        Self::with_cpu_preset("swiglu", Some(PairedEpiloguePreset::SwiGLU), |gate, up| {
            gate.silu() * up
        })
    }
    ...
    pub fn geglu() -> Self {
        Self::with_cpu_preset("geglu", Some(PairedEpiloguePreset::GeGLU), |gate, up| {
            gate.gelu() * up
        })
    }
    ...
    pub fn reglu() -> Self {
        Self::with_cpu_preset("reglu", Some(PairedEpiloguePreset::ReGLU), |gate, up| {
            gate.relu() * up
        })
    }
