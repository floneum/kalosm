...
pub(crate) fn q6k_ggml_dot_tiles(
    ...
    let low_word_offset = low_byte_offset >> 2u32;
    ...
    let high_word_offset = (high_byte_offset >> 2u32) + Q6K_QH_WORD_BASE;
    ...
    let scale_index = lane.ip.clone() * 8u32 + (lane.il.clone() >> 2u32);
    let scale_word0_offset = (scale_index.clone() >> 2u32) + Q6K_SCALE_WORD_BASE;
    ...
    let scale_lane0_shift = scale_lane0 << 3u32;
    let scale_lane1_shift = scale_lane1 << 3u32;
    let scale_byte = |word: &Tile, shift: &Tile| -> Tile {
        signed_byte_f32((word.clone() >> shift.clone()) & 0xffu32)
    };
    let scales = [
    ...
    for l in 0..4u32 {
        ...
        let q0 = (q1_byte.clone() & 0x0fu32) | ((qh_byte.clone() & 0x03u32) << 4u32);
        let q1 = (q2_byte.clone() & 0x0fu32) | ((qh_byte.clone() & 0x0cu32) << 2u32);
        let q2 = (q1_byte >> 4u32) | (qh_byte.clone() & 0x30u32);
        let q3 = (q2_byte >> 4u32) | ((qh_byte & 0xc0u32) >> 2u32);
