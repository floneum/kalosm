...
pub fn rms_norm_vec4<B>(
    ...
    phase.program_grid::<RMS_NORM_VEC4_BLOCK>([rows, 1, 1], |program| {
        ...
        if let Some(residual) = &residual {
            let residual_index = row.clone() * meta.residual_row_stride_vec + reduce_col.clone();
            value = value + program.load_vec4(residual.at(residual_index), reduce_mask, 0.0);
        }
        let dot = program.vec4_dot(value.clone(), value);
        let partial_sum = program.loop_fold(TileReduceOp::Sum, chunks, dot, TileLiteral::f32(0.0));
        ...
        for chunk in 0..chunks {
            ...
            if let Some(residual) = &residual {
                ...
                value = value + program.load_vec4(residual.at(residual_index), mask.clone(), 0.0);
            }
