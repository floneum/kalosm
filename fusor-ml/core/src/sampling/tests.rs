...
#[tokio::test]
async fn backend_mirostat2_sampler_matches_cpu_reference_for_sorted_top_k() {
    let device = Device::new().await.unwrap();
    ...
    let ids_data = TensorData::new_from_buffer(&device, id_buffer, &[ids.len()], DataTypeEnum::U32);
    let mu = 3.0;
    ...
    .unwrap();
    let result = Tensor::<1, u32>::from(output).as_slice().await.unwrap();
...
#[tokio::test]
async fn retry_status_does_not_mutate_mirostat_state() {
    let device = Device::new().await.unwrap();
    ...
    .unwrap();
    let result = Tensor::<1, u32>::from(output).as_slice().await.unwrap();
    let state = Tensor::<1, f32>::from(sampler.state.clone())
        .as_slice()
        .await
        .unwrap();
