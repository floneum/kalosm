...
impl GraphVisPass {
    fn visit_nary(&mut self, key: NodeIndex, operation: &crate::nary_wise::ElementwiseOperation) {
    ...
    fn visit_reduce(&mut self, key: NodeIndex, operation: &crate::ReduceOperation) {
        ...
        for input in &operation.inputs {
            let input = self.identities.get(input).unwrap();
            self.statements.push(Stmt::Edge(
    ...
    fn visit_view(&mut self, key: NodeIndex, operation: &crate::view::ViewOperation) {
        ...
        let output_layout = self.layout_pass.output_layout.get(&key).unwrap();
        let id = Identity::quoted(format!("view ({}) #{:?}", output_layout, key));
        self.statements.push(Stmt::Node {
            id: id.clone(),
            port: None,
            attr: None,
    ...
    pub(crate) fn graphvis(&self, root: NodeIndex) -> Graph {
        ...
        while let Some(node) = graph_vis_pass.queued.pop_front() {
            ...
            match &node_data.variant {
                ...
                ComputeGraphNodeVariant::QMatrix(op) => graph_vis_pass.visit_dequantize(node, op),
                ComputeGraphNodeVariant::Elementwise(op) => graph_vis_pass.visit_nary(node, op),
                ComputeGraphNodeVariant::Reduce(op) => graph_vis_pass.visit_reduce(node, op),
                ComputeGraphNodeVariant::View(op) => graph_vis_pass.visit_view(node, op),
                ComputeGraphNodeVariant::Assign(op) => graph_vis_pass.visit_slice_assign(node, op),
            }
        }
