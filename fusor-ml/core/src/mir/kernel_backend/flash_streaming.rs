use wgpu::naga::{
    AddressSpace, Arena, BinaryOperator, Binding, Block, BuiltIn, CollectiveOperation, EntryPoint,
    Expression, Function, FunctionArgument, GlobalVariable, Handle, LocalVariable, Module, Scalar,
    ShaderStage, Span, Statement, SubgroupOperation, Type, VectorSize,
};
...
impl FlashAttentionNagaBuilder {
    ...
    fn build(self) -> Option<Module> {
        ...
        let scratch_ty = constant_array_type(&mut module, f32_ty, BLOCK as u32, 4)?;
        ...
        let mask = self
            .has_mask
            .then(|| storage_global(&mut module, 3, storage_ty, true));
        let output_binding = if self.has_mask { 4 } else { 3 };
        let output = storage_global(&mut module, output_binding, storage_ty, false);
        ...
        let locals = FlashAttentionLocals {
            loop_idx: local(&mut function, u32_ty),
            score: local(&mut function, f32_ty),
            weighted: local(&mut function, f32_ty),
            m: local(&mut function, f32_ty),
            s: local(&mut function, f32_ty),
            o: local(&mut function, f32_ty),
        };
