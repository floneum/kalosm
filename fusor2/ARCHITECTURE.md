> Working target: `fusor2/`, this directory.
> Reference (read-only): `../fusor-ml`
> Acceptance target: betlang's `trainer/`, which is **not in this repository**. The gate below was enforced from `fusor2-conformance` while fusor2 lived beside that crate; the MSQ1 and trainer-gate cases did not survive the move here (see `fusor2-conformance/src/lib.rs`). The numbers stay as the design's stated bar; nothing in this checkout checks them.
...
fusor2-cpu         fearless_simd Target, SIMD emitter, CPU rules, worker pool      -> ir,tile,cost,gguf
fusor2             Tensor, ops, macro ops, layers, Graph, optimizers, Session      -> all
