//! Reverse mode as an L0 -> L0 transform.
...
/// A value on the tape. Ids are e-graph ids: forward and backward are one graph
/// with one root set, so gradient checkpointing is the extractor's
/// materialization bit rather than a pass.
pub type Val = Id;
...
pub trait Tape {
    ...
    /// The declared adjoint of `Gather`. Several lowerings coexist below it;
...
pub enum AdjointKind {
    ...
    /// reads `(window, step)`: at `step >= window` the adjoint is an
    /// elementwise mask-and-broadcast, and overlapping windows give
    /// `Scatter{Add}`.
    Structural,
}
...
/// tensor handle: a captured graph handle would close an `Arc` cycle pinning
