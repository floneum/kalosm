...
pub fn reshape_specs(
    ...
    while i < in_len || j < out_len {
        ...
        let j0 = j;
        let mut pi = in_shape[i].as_const().ok_or_else(|| reshape_symbolic(in_shape, out_shape))?;
        let mut pj = out_shape[j].as_const().ok_or_else(|| reshape_symbolic(in_shape, out_shape))?;
        i += 1;
        j += 1;
        while pi != pj {
            if pi < pj {
...
pub fn broadcast_shapes(a: &[Dim], b: &[Dim]) -> Result<Dims> {
    ...
    for i in 0..rank {
        ...
        out[i] = match (da, db) {
            ...
            (Some(x), Some(y)) => {
                return Err(Error::Shape(format!("cannot broadcast axis {i}: {x} vs {y}")));
            }
        };
    }
    Ok(out)
