...
mod tests {
    ...
    use crate::ir::launch::ScheduleDomain;
    ...
    /// Every `Fold` reachable from `id`'s class, including the ones minted
    ...
    fn folds_in(g: &EGraph, id: Id) -> Vec<(Id, Carrier, Vec<Operand>)> {
        ...
        while let Some(cur) = stack.pop() {
            ...
            seen.push(cur);
            if let Op::Launch(Launch::Fold { carrier, ops, .. }) = &g.node(cur).op {
    ...
    fn maps_in(g: &EGraph, id: Id) -> Vec<(Id, ScalarExpr, Vec<Operand>)> {
        ...
        for m in g.class_ids(g.class_of(id)) {
            if let Op::Launch(Launch::Map { body, ops, .. }) = &g.node(m).op {
    ...
    fn hoist_fires_on_a_saturated_scaled_reduction() {
        ...
        let Op::Launch(Launch::Fold { carrier, ops, .. }) = g.node(outer.2[0].src).op.clone() else {
    ...
    /// round-to-nearest, so the row carries `exact_in_float: true` and fires
    /// on a value whose `NumericContract` forbids reassociation.
    #[test]
    fn hoist_fires_under_strict_on_an_extremum_with_an_invariant_bias() {
        ...
        // A rounding body meets its value with `NumericContract::STRICT`, so
    ...
    /// `min_j(-x_j) == -max_j(x_j)`: the antitone row.
    #[test]
    ...
    /// `ValueFacts` has no sign lattice, so `max(sqrt x) = sqrt(max x)` and
    ...
    /// a per-row denominator held in a tensor, invariant along the reduced
    /// axis, hoists out of an additive fold.
    #[test]
    ...
    /// An additive fold is sign-blind, so any axis-invariant expression may
    /// be peeled; an extremum is not — `max(x * c) == max(x) * c` is false
    /// for `c < 0` — so a row over `Max`/`Min` admits only a `Lit`. Pins the
    /// guard: no extremum scaling row exists in `HOM_TABLE` yet.
    #[test]
    ...
    /// Every hoistable factor leaves in one firing.
    #[test]
    fn hoist_peels_every_factor_in_one_firing() {
        ...
        let Op::Launch(Launch::Fold { carrier, .. }) = g.node(outer.2[0].src).op.clone() else {
            panic!("the hoisted map must read a fold")
        };
    ...
    /// `prod_j exp(x_j) == exp(sum_j x_j)`: the `exp` row read outward, which
    /// also stops a product of exponentials leaving the representable range.
    #[test]
    fn hoist_turns_a_product_of_exponentials_into_one_exponential() {
        ...
        // Every partial product leaves the representable range; the sum never
        // does.
    ...
    /// The joint carrier a firing produced, if any, in the unpromoted
    /// spelling. `PROMOTE` also mints a `Vector`-slotted alternative beside
    /// it.
    ...
    /// `shift_stabilized_sum` oracle — including the `safe_delta` guard — so
    /// this asserts the law derives the hand-written algorithm.
    #[test]
    fn retarget_derives_the_shift_carrier_on_a_saturated_graph() {
        ...
        // Adding the two laws must not cost this chain its saturation.
        ...
        // retargeted chain; the depth the chain needs is pinned here so a
        ...
        let reaches = |from: Id| -> bool {
            ...
            while let Some(cur) = stack.pop() {
                ...
                seen.push(cur);
                // Only through views: a slot readback is one `Map { Arg(0) }`
                // over the joint fold.
                let view = match &g.node(cur).op {
                    Op::Union(..) => true,
                    Op::Logical(Logical::Restride { .. }) => true,
    ...
    /// weight sits on a multiplicative path outside the shifted term, so this
    /// hangs on `linear_factor` — a root-only peel matches nothing here.
    #[test]
    fn retarget_fires_on_a_weighted_log_sum_exp() {
        ...
        // `L(1) = w`: the element enters as its own weight and no multiply by
        // one survives.
    ...
    /// `max_j(w_j + x_j - max_k x_k)` is a tropical reduction and fires at
    /// the `max-plus` row with no exponential anywhere.
    #[test]
    ...
    /// strip-mining law must decline because a symbolic extent has no
    /// blocking factor. The result is single-pass and unblocked.
    #[test]
    fn retarget_fires_over_a_symbolic_reduction_length() {
        ...
        let Op::Launch(Launch::Fold { space, .. }) = &g.node(joint).op else {
            panic!()
        };
    ...
    /// pulls that producer into its own lift and stops naming the
    /// intermediate, so the two element expressions must be compared on the
    /// common basis `common_basis` establishes.
    #[test]
    ...
    /// One row has to cover every retargeted slot at once, and when it does a
    /// plain running sum and a weighted one are rescaled by the same factor.
    /// The program is a self-normalized importance-sampling estimator: the
    /// partition function `sum exp(s)` and the estimate `sum v * exp(s)`.
    #[test]
    fn one_row_retargets_every_slot_of_a_multi_slot_accumulator() {
        ...
        // Slot 1 and slot 2 differ only in what they carry, never in how they
        // are rescaled.
        let w = c.width();
    ...
    /// rescaling half an accumulator: slot 1 accumulates with `Max` while
    /// slot 0 accumulates with `Add`.
    #[test]
    ...
    /// When the shift comes from a supplied buffer rather than a fold over
    /// this axis, the structural condition fails and the rule declines.
    #[test]
    ...
    /// Every guard compares modulo commutation: a commutative binop's
    ...
    fn every_guard_matches_modulo_commutation() {
        ...
        // The shift template, whose `u - rho` is not commutative but sits
        // under commutative parents.
    ...
    fn every_retarget_row_declares_its_own_action() {
        for row in RETARGET_TABLE {
            ...
            // `T` composes with the module's own accumulation the way the
            // action says it does.
            assert!(
    ...
    /// Pins that the set of rows the rule uses is exactly the set that is
    /// covered by a firing test above.
    #[test]
