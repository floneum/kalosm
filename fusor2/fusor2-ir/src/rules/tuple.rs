...
mod tests {
    ...
    fn base_fold(g: &EGraph, mut id: Id) -> Option<Id> {
        loop {
            match &g.node(id).op {
                Op::Logical(Logical::Restride { x, .. }) => id = *x,
                Op::Launch(Launch::Fold { .. }) | Op::Logical(Logical::Fold { .. }) => return Some(id),
                _ => return None,
            }
        }
    }
    ...
    /// the same multi-slot `Fold` under both.
    ///
    /// Every hit is checked against the carrier obligation and one `post` per
    /// slot, so no positive test can pass on a node the verifier would
    /// reject.
    fn joined_under(
        g: &EGraph,
        consumer: Id,
        ...
        for m in g.chain(consumer) {
            let ops = match &g.node(m).op {
                Op::Launch(Launch::Map { ops, .. }) | Op::Launch(Launch::Fold { ops, .. }) => ops.clone(),
                _ => continue,
            };
            let (Some(fa), Some(fc)) = (
    ...
    // The layer_norm/rms_norm backward: the composed adjoint emits `sum(dy)`
    // and `sum(dy * xhat)` over the same feature axis of the same operands,
    // meeting at the `dx` expression.
    ...
    /// Fires on a real saturated graph.
    #[test]
    ...
    /// Each slot of the joined nest computes exactly what its own fold
    /// computed alone, against a host reference.
    #[test]
    ...
    // Min and max in one pass: the two statistics share no algebra; the law
    // joins them because they fold one axis of one operand.
    ...
    /// TUPLE carries no `reassoc` guard: every slot folds in precisely the
    /// order it folded alone.
    #[test]
    fn tuple_fires_under_a_strict_numeric_contract() {
        let mut g = ts::graph();
        let shape = [Dim::Const(4), Dim::Const(6)];
        let raw = ts::buffer(&mut g, Dtype::F32, &shape);
        // A rounding body forces `reassoc: false`.
        let q = ts::map(
            &mut g,
    ...
    /// tests that a view spine is re-applied over the readback: the count is
    /// read through a two-step narrow-then-drop chain.
    #[test]
    ...
    fn strided(src: Id, shape: &[Dim], strides: &[Dim]) -> Operand {
        Operand {
            ...
            access: crate::ir::launch::AccessPlan::Alias,
        }
    }

    /// A `[Scalar]` row statistic and the `[Vector(Dh)]` accumulator that
    /// reads it are one nest: `l[q] = sum_k P` at `axis = 1` of `[Lq, Lk]`
    /// against `o[q,d] = sum_k P*V` at `axis = 2` of `[Lq, Dh, Lk]` with `Dh`
    /// promoted. The reduced axis's iteration index is 1 both ways.
    ///
    /// The row sum's `P` edge is only legal on the joint restated at stride 0
    /// along `Dh` — which also makes it the same edge the output accumulator
    /// already reads, so the joint reads `P` once.
    #[test]
    fn tuple_joins_a_promoted_nest_with_an_unpromoted_one() {
        ...
        // The joint is minted in the promoted domain.
        ...
        // `P` was widened at stride 0 along `Dh`, so the joint reads it once.
        ...
        // A `Vector` slot merges positionwise, so folding the carrier over
        // one promoted position is that position's accumulator. The rows are
        // `[P, V]` — the order `unify_ops` put them in.
    ...
    /// Two different promotions of one space have no common nest: the joint
    /// would have to hold two carrier geometries at once.
    #[test]
    fn tuple_declines_two_different_promotions() {
        ...
        // Same space and same reduced axis, but one promotes axis 1 and the
        // other axis 0 — two geometries.
    ...
    /// are still one join, and the joint takes neither side's domain.
    #[test]
    fn the_joint_takes_neither_sides_schedule_domain() {
        ...
        let tiled = ScheduleDomain::Fold(crate::ir::launch::FoldDomain {
            strategies: smallvec![
                crate::ir::launch::FoldStrat::Subgroup,
                crate::ir::launch::FoldStrat::WgTree { lane_group: 64 },
            ],
        });
        ...
        assert_eq!(carrier.width(), 2);
        let Op::Launch(Launch::Fold { sched, .. }) = g.node(joint).op.clone() else {
            unreachable!()
        };
        assert_eq!(
            sched,
    ...
    fn a_slot_readback_carries_the_side_it_replaces_facts() {
        ...
        let shape = [Dim::Const(4), Dim::Const(6)];
        let x = ts::buffer(&mut g, Dtype::F32, &shape);
        // `ts::fold` is the Logical spelling; `ts::kfold` the Launch one.
        let a = ts::fold(
            &mut g,
            ts::binop_carrier(BinOp::Add, Dtype::F32),
            1,
    ...
    /// A carrier too wide to hold in registers is unschedulable, not merely
    /// slower, so the footprint is a legality guard.
    #[test]
    ...
    /// inner nests over one axis joins them exactly as a `Map` consumer does.
    #[test]
    ...
    fn a_third_nest_joins_onto_the_result() {
        ...
        let widest = g
            ...
            .filter_map(|m| match &g.node(m).op {
                Op::Launch(Launch::Map { ops, .. }) => Some(ops.clone()),
                _ => None,
            })
            .filter_map(|ops| {
                let f = base_fold(&g, ops.first()?.src)?;
                match g.node(f).op.clone() {
                    Op::Launch(Launch::Fold { carrier, .. }) => Some(carrier.width()),
                    _ => None,
                }
            })
            .max()
