...
/// expression from `tanh`, not an alias: it is needed where a driver's native
/// tanh under-saturates the GELU tail.
...
/// `less_approximate_exp` are the other two of the 23; they are not `exp`, so
/// they get a relative bound instead.
#[rustfmt::skip]
...
fn scalar_arith() -> Vec<(&'static str, Domain, UnaryOp, fn(f32) -> f32)> {
    vec![
        ...
        // adjoint alone; `forward_only()` below covers the forward half.
...
/// sample away from: `abs` on a negative, and either clamp actually clamping.
/// No gradient is taken, so the kink is harmless, and [`non_vacuous`] keeps the
/// domain honest.
#[rustfmt::skip]
...
/// Refuse a row whose reference is the identity on its own sampled data: such
/// a case passes against a passthrough implementation and reports as coverage.
...
/// The 6 scalar comparisons, plus the 6 fusor-core-compatible aliases
/// (`eq`/`ne`/`lt`/`lte`/`mt`/`mte`), which take a *scalar*, not a tensor.
/// Each differentiates to zero.
#[rustfmt::skip]
fn scalar_comparisons() -> Vec<(&'static str, UnaryOp, fn(f32) -> f32)> {
    vec![
        ...
        ("eq_alias",   |x| x.eq(0.0),        |v| f32::from(v == 0.0)),
        ("ne_alias",   |x| x.ne(0.0),        |v| f32::from(v != 0.0)),
        ("lt_alias",   |x| x.lt(0.0),        |v| f32::from(v < 0.0)),
        ("lte_alias",  |x| x.lte(0.0),       |v| f32::from(v <= 0.0)),
        ("mt",         |x| x.mt(0.0),        |v| f32::from(v > 0.0)),
        ("mte",        |x| x.mte(0.0),       |v| f32::from(v >= 0.0)),
    ]
}
...
pub fn cases() -> Cases {
    ...
    // The two GPU-approximate exponentials are not `exp`, so they get a
    // relative bound rather than an elementwise reference.
    ...
    // The operator surface. A chained expression is a different
    // `ScalarExpr::compose` shape than a single op.
...
/// A rank-2 activation against a rank-1 operand, right-aligned. No implicit
/// broadcasting exists at L0 (the frontend emits `Restride { multiplier: 0 }`),
/// so this checks the frontend's right-aligned rules and that a stride-0 axis's
/// adjoint is a sum over that axis.
...
///
/// If `fusor2::Tensor` does not expose the entry point, the case reports it
/// missing by name rather than aliasing to `exp` and passing vacuously.
...
/// `UnOp::LessApproximateExp`), not aliases for `exp`.
...
mod tests {
    ...
    /// The guard fires on the domains the backward rows sample.
    #[test]
    ...
    /// The guard does not fire on the widened forward-only domains.
    #[test]
    ...
    fn all_twelve_comparisons_plus_the_four_extras_are_registered() {
        ...
        assert_eq!(
            ...
            12,
            "6 scalar forms plus the 6 reference aliases, which also take a scalar"
        );
