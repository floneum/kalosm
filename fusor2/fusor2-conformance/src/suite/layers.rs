...
mod tests {
    ...
    fn the_host_linear_is_x_times_w_transposed() {
        // A hand-worked 1x2 @ (2x2)^T.
        let x = [1.0f32, 2.0];
        let w = [3.0f32, 4.0, 5.0, 6.0]; // [out=2, in=2]
        let out = host_linear(&x, &w, None, 1, 2, 2);
    ...
    fn the_bias_enters_every_row_once() {
        ...
        let b: Vec<f32> = (0..4).map(|o| o as f32).collect();
        let out = host_linear(&x, &w, Some(&b), 2, 3, 4);
        for r in 0..2 {
            for o in 0..4 {
                assert_eq!(out[r * 4 + o], o as f32);
            }
        }
    }

    #[test]
    fn the_sampled_tokens_always_repeat() {
        // Otherwise `embedding_layer_backward` would prove nothing.
        for seed in 0..8 {
            let tokens = sample_tokens(seed, 4, 5);
            assert_eq!(tokens[0], tokens[1], "{tokens:?} has no forced repeat");
            assert!(tokens.iter().all(|t| *t < 5));
        }
    }
}
