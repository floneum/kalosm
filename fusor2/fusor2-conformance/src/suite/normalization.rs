...
//! rms_norm spellings are the *same* expansion behind different sugar, so a
//! case that passes for `softmax_slow` and fails for `softmax` is evidence
//! that the macro node and its `defn` are not in one e-class — which is the
//! one thing the design says cannot happen.
//!
//! Owned by W14.
