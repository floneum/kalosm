...
mod tests {
    ...
    #[test]
    fn the_permutation_reference_is_an_actual_permutation() {
        let shape = &[2u64, 3, 4];
        let data: Vec<f32> = (0..24).map(|i| i as f32).collect();
        for perm in [[2usize, 0, 1], [2, 1, 0], [0, 2, 1], [0, 1, 2]] {
            let (out_shape, mut values) = ref_permute(&data, shape, &perm);
            assert_eq!(
                out_shape,
                perm.iter().map(|&p| shape[p]).collect::<Vec<_>>()
            );
            values.sort_by(f32::total_cmp);
            assert_eq!(values, data, "a permutation reference lost or gained values");
        }
    }

    #[test]
    fn the_window_reference_repeats_exactly_where_the_windows_overlap() {
        let shape = &[2u64, 3, 4];
        let data: Vec<f32> = (0..24).map(|i| i as f32).collect();
        // Step == window: every element appears once.
        let (out_shape, values) = ref_windows(&data, shape, 2, 2);
        assert_eq!(out_shape, vec![2, 3, 2, 2]);
        assert_eq!(values.len(), 24);
        // Step < window: 3 positions x 2 wide over an extent of 4, so 6 reads
        // per row rather than 4.
        let (out_shape, values) = ref_windows(&data, shape, 2, 1);
        assert_eq!(out_shape, vec![2, 3, 3, 2]);
        assert_eq!(values.len(), 36);
        // Window 3, step 1: 2 positions x 3 wide.
        let (out_shape, values) = ref_windows(&data, shape, 3, 1);
        assert_eq!(out_shape, vec![2, 3, 2, 3]);
        assert_eq!(values.len(), 36);
    }

    #[test]
    fn the_pad_reference_inserts_exactly_the_requested_zeros() {
        let shape = &[2u64, 3, 4];
        let data: Vec<f32> = (1..=24).map(|i| i as f32).collect();
        let (out_shape, values) = ref_pad(&data, shape, 2, 1, 2);
        assert_eq!(out_shape, vec![2, 3, 7]);
        assert_eq!(
            values.iter().filter(|v| **v == 0.0).count(),
            18,
            "3 zeros per row"
        );
        let (out_shape, values) = ref_pad(&data, shape, 0, 1, 1);
        assert_eq!(out_shape, vec![4, 3, 4]);
        assert_eq!(values.iter().filter(|v| **v == 0.0).count(), 24);
    }

    #[test]
    fn the_slice_reference_keeps_exactly_the_requested_run() {
        let shape = &[2u64, 3, 4];
        let data: Vec<f32> = (0..24).map(|i| i as f32).collect();
        let (out_shape, values) = ref_slice(&data, shape, 2, 1, 2);
        assert_eq!(out_shape, vec![2, 3, 2]);
