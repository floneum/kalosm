...
fn narrow_sym(t: &Tensor, axis: usize, len: Dim) -> fusor2::Result<Tensor> {
    let shape = t.shape();
...
fn main() -> fusor2::Result<()> {
    ...
    let knew = graph.leaf(
        "k_new",
        &[Dim::Const(1), Dim::Const(HKV), Dim::Const(1), Dim::Const(DH)],
        Dtype::F32,
    )?;
    let vnew = graph.leaf(
        "v_new",
    ...
    for step in 0..steps {
        ...
        assert!(max_err < 1e-4, "step {step} diverged: {out:?} vs {expect:?}");
