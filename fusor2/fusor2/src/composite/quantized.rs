...
//!
//! A block decode is bit arithmetic over a strided view of a byte buffer, so it
//! is two core nodes that fusion, the contraction lowering, the cooperative
//! staging fill and the cost model consume with no quantization-aware code.
//!
//! The block stream is bound as a rank-1 `U32` leaf, so every field a decode
//! reads has to be a whole `u32` word of it, which makes the block stride the
//! gate. [`QLayout::F32Scales`] is word-aligned in every format:
//! 20/24/36/148/180/212 bytes. Of the native strides only Q4K's 144 and Q5K's
//! 176 are; Q4_0's 18, Q5_0's 22, Q8_0's 34 and Q6K's 210 leave two bytes over,
//! so a `Restride` cannot address them and they keep `BlockProgram::Native`,
//! bridged by `qrepack`.
//!
//! An f16 scale is not part of that gate: [`f16_lane`] decodes a half out of a
//! word in pure `ScalarExpr` arithmetic, at the 2-aligned half-word offset
//! [`scale_field`] gives.
//!
//! Block strides come from `QFmt::block_bytes` and field offsets from
//! `fusor2_gguf::blocks::block_fields`; no format constant is written here.
...
///
/// An f16 field only has to be 2-aligned, so this is not [`field_word`].
...
/// `(q - bias) * bitcast<f32>(scale_word)`: the tail every block format shares
/// once its quant is an unsigned integer in a register.
...
/// arithmetic: with `h = (word >> 16*lane) & 0xFFFF`, `s = (h >> 15) & 1`,
/// `e = (h >> 10) & 0x1F` and `m = h & 0x3FF`, the value is
/// `(1 - 2s) * exp2(e - 15) * (1 + m/1024)`. `e == 0` takes the subnormal row
/// `2^-14 * (m/1024)` with no implicit leading one, so an all-zero half decodes
/// to `0.0` rather than 3.05e-5. Inf and NaN (`e == 31`) are not representable
/// scales and are not special-cased.
...
///
/// On `None` the class is left as it is and the pair keeps its `BlockProgram`:
/// the four `Native` rows whose block stride is not a whole number of words.
pub(crate) fn dequant_defn(q: &QMatrix) -> Result<Option<Id>> {
    ...
    // No layout guard: an f16 scale decodes through `f16_lane`, so the only
    // restriction is the word-alignment test below.
    ...
    // `constant_leaf` is content-addressed, so buffers with the same bytes
    // share one node.
    ...
    // The `(q - bias) * d` tail the three 32-element formats share.
    ...
    let built = match fmt {
        ...
        QFmt::Q8_0 => {
            ...
            // which this decode must clear to reach a staging fill.
        ...
        // 148 bytes = 37 words: f32 `d` at word 0, f32 `dmin` at word 1, twelve
        // packed six-bit group scale/min bytes in words 2..5, 128 nibble-packed
        // quant bytes in words 5..37. Element `e = gp*64 + parity*32 + w*4 + b`,
        // so group `g = gp * 2 + parity` and the quant byte is `gp*32 + w*4 + b`.
        // `lane = g & 3` and `high = g >= 4` depend on `(gp, parity)` alone and
        // are eight-entry index leaves. Q5K is 180 bytes = 45 words: the same
        // fields with a 32-byte high-bit plane ahead of the quants, where `e`'s
        // fifth bit is bit `g` of plane byte `w * 4 + b`, shift `8 * b + g`.
        QFmt::Q4K | QFmt::Q5K => {
            ...
            // `decode_k::k4_group_scale_min`, op for op.
            let gs = pick(
            ...
            // At `Native` both scales are f16 lanes of the block's first word,
            // `d` in lane 0 and `dmin` in lane 1, so operands 0 and 1 are the
            // same restride and only the lane differs.
            let d = scale_of(arg(0), scale_lane);
            let dmin = scale_of(arg(1), min_lane);
            // `k4_lane`'s operation order, `q * (gs * d) - (gm * dmin)`, is not
            // reassociated: the result is bit-identical to
        ...
        // 212 bytes = 53 words: 128 low-plane bytes, 64 high-bit bytes, sixteen
        // signed i8 group scales, then the f32 super-block scale at word 52.
        // Splitting `e = chunk * 128 + group * 32 + hb` as
        // `group = gh * 2 + gl` and `hb = wh * 16 + wl * 4 + b` makes every
        // address a plain stride over `[chunk 2, gh 2, gl 2, wh 2, wl 4, b 4]`:
        ...
        QFmt::Q6K => {
            ...
            let d = scale_of(arg(0), scale_lane);
            // `d * group_scale * q` in `cpu_dequantize_block`'s association, so
            // the result is bit-identical to the oracle.
            let body = ScalarExpr::bin(
                BinOp::Mul,
...
mod tests {
    ...
    /// lanes, through the real evaluator. `e == 31` is Inf/NaN: not a
    /// representable scale, and NaN is unequal to itself, so those 2,048
    /// patterns are excluded rather than asserted on.
    #[test]
    fn f16_lane_decodes_every_finite_half_exactly() {
        ...
        for lane in [0u32, 1] {
            ...
            for (g, b) in got.iter().zip(&bits) {
                // Exact: a half is representable in f32, and the arithmetic is
                // a power of two times a sum of exact dyadic rationals.
