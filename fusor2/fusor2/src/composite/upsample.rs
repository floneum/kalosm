...
fn blend(t: &mut GraphTape<'_>, a: Val, b: Val, w: Val, axis: u32) -> Result<Val> {
    ...
    let mut specs: SmallVec<[fusor2_ir::shape::StrideSpec; 6]> = SmallVec::new();
    for (i, d) in shape.iter().copied().enumerate() {
        if i == axis as usize {
            specs.push(fusor2_ir::shape::StrideSpec::dim(0, d));
        } else {
            specs.push(fusor2_ir::shape::StrideSpec::broadcast(d));
        }
    }
...
mod tests {
    ...
    #[test]
    fn nearest_indices_repeat_each_source_position_exactly() {
        assert_eq!(nearest_indices(2, 4), vec![0, 0, 1, 1]);
        assert_eq!(nearest_indices(3, 9), vec![0, 0, 0, 1, 1, 1, 2, 2, 2]);
    }

    #[test]
    fn upsample_nearest2d_scales_both_axes() {
        let g = graph();
        let x = leaf(&g, &[1, 3, 4, 5]);
        let y = upsample_nearest2d(&x, 2, 3).unwrap();
        assert_eq!(
            &g.handle().facts(y.id()).shape[..],
            &[Dim::Const(1), Dim::Const(3), Dim::Const(8), Dim::Const(15)]
        );
    }

    #[test]
    fn a_unit_scale_is_still_a_legal_upsample() {
        let g = graph();
        let x = leaf(&g, &[1, 1, 2, 2]);
        let y = upsample_nearest2d(&x, 1, 1).unwrap();
        assert_eq!(
            &g.handle().facts(y.id()).shape[..],
            &[Dim::Const(1), Dim::Const(1), Dim::Const(2), Dim::Const(2)]
        );
        assert!(upsample_nearest2d(&x, 0, 1).is_err());
    }

    #[test]
    fn bilinear_taps_bracket_every_output_position() {
        let (lo, hi, frac) = bilinear_taps(4, 8, false);
        assert_eq!(lo.len(), 8);
        for i in 0..8 {
            assert!(hi[i] >= lo[i] && hi[i] <= 3);
            assert!((0.0..=1.0).contains(&frac[i]));
        }
        // align_corners pins both endpoints.
        let (lo, hi, frac) = bilinear_taps(4, 7, true);
        assert_eq!(lo[0], 0);
        assert!(frac[0].abs() < 1e-6);
        assert_eq!(hi[6], 3);
    }

    #[test]
    fn bilinear_preserves_rank_and_resamples_the_last_two_axes() {
        let g = graph();
        let x = leaf(&g, &[2, 3, 4, 4]);
        let y = upsample_bilinear(&x, &[Dim::Const(8), Dim::Const(8)], false).unwrap();
        assert_eq!(
            &g.handle().facts(y.id()).shape[..],
            &[Dim::Const(2), Dim::Const(3), Dim::Const(8), Dim::Const(8)]
        );
    }

    #[test]
    fn shrinking_is_refused_because_that_is_a_pool() {
        let g = graph();
        let x = leaf(&g, &[1, 1, 8, 8]);
        assert!(upsample_nearest(&x, &[Dim::Const(4), Dim::Const(4)]).is_err());
    }
}
