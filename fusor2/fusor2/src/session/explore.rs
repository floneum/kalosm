...
mod tests {
    ...
    /// The counter is per key and deterministic: the same resolve sequence
    /// explores at the same steps, every run.
    #[test]
    fn the_epsilon_counter_is_per_key_and_deterministic() {
        let mut st = ExploreState::default();
        for i in 1..=40u64 {
            assert_eq!(st.key_mut(key(1)).counter + 1, i);
            st.key_mut(key(1)).counter += 1;
        }
        // A second key starts its own schedule.
        assert_eq!(st.key_mut(key(2)).counter, 0);
    }

    /// State for evicted replay keys does not accumulate.
    #[test]
    fn key_state_is_fifo_bounded() {
        let mut st = ExploreState::default();
        for i in 0..(KEY_CAP as u64 + 10) {
            st.key_mut(key(i)).counter += 1;
        }
        assert_eq!(st.keys.len(), KEY_CAP);
        assert_eq!(st.order.len(), KEY_CAP);
        // The oldest keys are the evicted ones.
        assert!(!st.keys.contains_key(&key(0)));
        assert!(st.keys.contains_key(&key(KEY_CAP as u64 + 9)));
    }
}
