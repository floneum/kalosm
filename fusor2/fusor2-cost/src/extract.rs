...
use fusor2_ir::extract::{ExtractBudget, Extraction, Extractor, Dispatch, Plan};
...
// Dumps every launch of every extracted plan when `FUSOR2_DUMP_PLAN` is set,
    ...
    if std::env::var_os("FUSOR2_DUMP_EDGES").is_some() {
        for (i, l) in plan.launches.iter().enumerate() {
            ...
            let ident = match &n.op {
                Op::Launch(fusor2_ir::ir::launch::Launch::Map { ops, body, .. }) => {
                    ops.len() == 1 && format!("{body:?}").starts_with("ScalarExpr(ScalarNode { kind: Arg(0)")
                }
                _ => false,
            };
            let kind = match &n.op {
                Op::Launch(fusor2_ir::ir::launch::Launch::Map { .. }) => "Map",
                Op::Launch(fusor2_ir::ir::launch::Launch::Fold { .. }) => "Fold",
                Op::Launch(fusor2_ir::ir::launch::Launch::Contract { .. }) => "Contract",
                Op::Launch(fusor2_ir::ir::launch::Launch::Gather { .. }) => "Gather",
                Op::Launch(fusor2_ir::ir::launch::Launch::Scatter { .. }) => "Scatter",
                Op::Launch(fusor2_ir::ir::launch::Launch::Region { .. }) => "Region",
                Op::Launch(fusor2_ir::ir::launch::Launch::Ext { .. }) => "Ext",
...
fn op_tag(op: &Op) -> String {
    ...
    match op {
        Op::Launch(Launch::Map { space, ops, body, .. }) => {
            let srcs: Vec<String> = ops
                ...
                .map(|o| {
                    format!(
                        ...
                        o.src,
                        match &o.access {
                            fusor2_ir::ir::launch::AccessPlan::Alias => "",
                            fusor2_ir::ir::launch::AccessPlan::Gather => ":G",
                            fusor2_ir::ir::launch::AccessPlan::Pack { .. } => ":P",
                            fusor2_ir::ir::launch::AccessPlan::Unflatten(_) => ":U",
                        },
                        o.layout.offset()
                    )
                })
...
fn seed_theta_trailed(
    ...
    for id in selected {
        ...
        if matches!(domain, ScheduleDomain::Point) {
            let prev = ex
                .theta
                .insert(id, fusor2_ir::ir::launch::SchedPoint::Point);
            if prev != Some(fusor2_ir::ir::launch::SchedPoint::Point) {
...
struct RepairTrail {
    ...
    theta: SmallVec<[(Id, Option<fusor2_ir::ir::launch::SchedPoint>); 8]>,
}
...
fn body_digest(op: &Op) -> u64 {
    use fusor2_ir::ir::launch::Operand;
...
mod tests {
    ...
    /// `realize::is_self_referential` cannot see: neither member names its own
    ...
    fn launch_cut_at_index_space_mismatch() {
        // Map[1024] feeding Fold[1024 -> 64]: the fold's space does not
    ...
    fn budget_is_respected() {
        ...
        // The budget is realized node visits, not a wall clock: a deadline
        // would make the winning plan depend on machine load, and the plan is
        // a cross-process cache key. The 3,000-deep chain is the pathological
        // shape (one launch with 3,000 members and 3,000 singleton classes).
        assert!(
        ...
        // same two bounds are asserted against its own counter.
        assert!(
    ...
    fn remat_prices_exactly() {
        ...
        // roofline's `max` hides under the consumers' own bandwidth.
        ...
        // The search prices it as a win, above. Whether it may ship it is a
        // separate question: a launch is lowered from one node, so an inlined
        // producer that no rule folded into its consumer would leave that
        // kernel reading an operand nothing wrote. The pricing assertions
        // above pin the remat term itself.
        let plan = s
            .extract(&g, &roots, &cost, ExtractBudget::default())
            .unwrap();
        assert!(
