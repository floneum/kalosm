...
pub fn lower_contract(
    ...
    match (family, theta) {
        ...
        (Family::GenericFold, theta) => lower_generic(ctx, op, theta).map(|k| vec![k]),
...
pub fn lower_sgemv(mut ctx: Ctx<'_>, op: &L1, p: SgemvParams) -> Result<KernelIr> {
    ...
    let mut body: Vec<Stmt> = Vec::new();
    let lane = ctx.b.builtin(Builtin::Lane);
    let row = ctx.b.builtin(Builtin::ProgramId(WorkgroupAxis::X));
    ...
    let grid = distribute_workgroups(
        shape.m.saturating_mul(shape.batch).max(1),
