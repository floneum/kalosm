...
//!
//! [`ScatterMode::WgPrivateMerge`] is the body that deletes the trainer's
//! host-side three-level sorted fold: each workgroup zeroes a private
//! `[bins x elem]` tile, accumulates its slice of the updates into it with no
//! atomics, barriers, and then issues one atomic add per nonzero bin.
//! Legality (`bins * elem_bytes <= max_compute_workgroup_storage_size`) was
//! already checked by W1, so nothing here re-checks it and nothing falls back.
//!
...
/// The four `ScatterMode`s name one map and differ only in *strategy*, and on
/// this runtime they all lower through one nest: **one lane per output
